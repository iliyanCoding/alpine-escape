...
function init() {
  ...
  loadAssets([
    { var: terrain.image, url: "/static/assets/tilemap_packed.png" }
  ], gameLoop);
...
function updatePlaying() {
  ...
  for (const row of terrain.rows) {
    if (row.worldY <= playerWorldY && playerWorldY < row.worldY + GAME.TILE_SIZE) {
      player.onTrack = playerCol > row.trackLeft && playerCol < row.trackRight;
      break;
    }
  }
  ...
  for (const obs of obstacles.obstacles) {
    ...
    if (!player.invincible && player.hitCooldown <= 0 && collides(player, { x: obs.x, y: screenY, width: obs.width, height: obs.height })) {
      player.hp -= 1;
  ...
  for (let i = turrets.snowballs.length - 1; i >= 0; i--) {
    ...
    if (!player.invincible && player.hitCooldown <= 0 && collides(player, screenS)) {
      player.hp -= 1;
      player.hitCooldown = 120;
      player.hit();
  ...
  for (let i = wolves.wolves.length - 1; i >= 0; i--) {
    ...
    if (!player.invincible && player.hitCooldown <= 0 && collides(player, screenW)) {
      player.hp -= 1;
      player.hitCooldown = 120;
      player.hit();
